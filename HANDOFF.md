...
6. **Wire the CJ merchants for the originals side — IN PROGRESS, waiting on CJ.** The site says "Nx cheaper" but cannot link the original at all: **all 200 references declare `affiliateLinkId: "original-<slug>"` and not one resolves**, so every one falls through to the placeholder branch and `hasRealAffiliateLink()` suppresses the buy-original button everywhere. My Perfume Shop has been closed for tracking since 1 Sep.

   **Our CJ advertiser is `fragranceshop.com`** — a US discount retailer of GENUINE designer fragrances (New Jersey, trading since 1998; grey-market/parallel import, which is how the prices are low). **It is NOT `thefragranceshop.com`**, the alcohol-free-oil dupe house an earlier note confused it with — unrelated companies, near-identical names. Perfumania is the second CJ approval, also originals-side. **Verify a merchant against the domain in the CJ advertiser record, never a name search.**

   Wiring it closes four gaps at once: real `original-*` links (buy-original button on catalog *and* dupe comparison), the **44 references with no image**, a re-source for the **156 images** still tied to the dead My Perfume Shop programme, and real original prices instead of hand-typed constants that go stale silently.

   **Feed status, 5 Sep: created, not yet delivered.** Shopping/Google format, TAB-delimited, `AdvertiserName-FeedName-shopping.txt.zip`. CJ generates on its own schedule and sends a *separate* "ready" notification — before that only the format sample downloads, which is not a fault. Two settings to change in Account → Subscriptions first:

   - **Transport → CJ HTTP/S.** CJ SFTP is unusable from modern tooling: `datatransfer.cj.com` runs Apache SSHD 2.5.1 offering **only `ssh-dss`** host keys. OpenSSH removed DSA outright (compiled out on this machine) and recent paramiko dropped it — the failure is `no matching host key type found. Their offer: ssh-dss`. curl's libssh2 still speaks DSA but needs a `known_hosts` entry that no keyscan tool will produce, because they all refuse DSA. Tested 5 Sep; don't re-litigate it.
   - **Schedule → Daily.** "When updates occur" only fires when the advertiser next imports a catalogue, which could be days.

   TAB delimiting is the right choice rather than a problem — fragrance descriptions are dense with commas (note pyramids read "bergamot, jasmine, sandalwood"), so tabs dodge the CSV quoting fragility. **The CJ schema is nothing like Awin's**: 66 uppercase Google-Shopping columns (`ID`/`TITLE`/`LINK`/`PRICE`/`IMAGE_LINK`) against Awin's `aw_product_id`/`product_name`/`merchant_deep_link`/`search_price`/`merchant_image_url`, so every ingest and matching script needs a mapping layer before it can read one. A format sample is saved at `scripts/feeds/sample-shopping.csv` (gitignored) for writing it against. `SUB_ID_PARAM` already maps `cj` → `sid`, so the link plumbing itself is ready.
