...
9. **Dashboard/report sync** — confirm `internal/dashboard/design/Main.dc.html` actually reflects current reality: every task a department has been assigned or has completed (per its `CLAUDE.md`, its `reports/` folder, or recent git history) shows up in the task list with correct status, and every report meant to be founder-facing (a completed gap-analysis, weekly accounting/control report, etc.) is linked from that department's card via a `realReport` entry, not left sitting unlinked in a `reports/` folder. Also confirm the *published* dashboard artifact was actually re-seeded and republished after the repo file changed, not just edited locally — a repo edit with no matching republish is a gap this check exists to catch (compare `Main.dc.html`'s git-log timestamp against the artifact's last-updated time via the `Artifact` tool's `action: "list"`). This department has no `Artifact`-publish tool itself, so it flags the gap for whoever holds the URL to fix, same as any other finding.

## Standing orders (from the founder, 2026-08-19; check 9 added 2026-08-20)

Ongoing mandate, not a one-time task: keep Sirketim efficient, cost-effective, and high-quality by running checks 6–9 above as a standing part of every audit pass, not just the five original checks. First concrete pass should cover:
...
- Confirm every department card on the dashboard with a completed report on disk (e.g. architecture's `cad-tooling-gap-analysis.md`) has a matching `realReport` link, and that every in-progress/completed task referenced in a department's recent work is actually present in the dashboard's task list — this check was added after exactly that gap was found and fixed manually (architecture's task 122 and its report link) during the 2026-08-20 session that set up this standing order.
