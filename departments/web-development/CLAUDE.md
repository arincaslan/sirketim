...
- **`motion`** (`https://mcp.motion.so/mcp`, OAuth-style like `openart`/`fanvue` — no API key committed) — added 2026-08-25, granted to `web-developer` as `mcp__motion`. Motion is an AI motion-graphics agent: full text-prompt-to-finished-video production (storyboarding, motion graphics, editing), confirmed to expose an MCP server (`docs.motion.so/guides/mcp`, "Call Motion from Claude... and it renders the video for you"). Added directly at the founder's request, after `web-developer` built a hand-drawn/hand-coded SVG illustration (a line-art atomizer, for a DRYDOWN homepage scroll animation) that the founder rejected outright ("this is bad dont ever do that svg thing" — see memory `feedback_no_handdrawn_svg_illustration` if using memory, or this file's own standing rule below) — the intent is a proper AI-generated motion-graphics video instead of hand-coded vector art for that kind of visual.
  - **Cost genuinely unconfirmed** — unlike `openart`/`chrome-devtools` (free or flat small subscription, already budgeted in the ledger), Motion's homepage doesn't publish pricing; it gates behind a demo/signup, and language like "Studio Service for high-stakes launches" suggests it may be priced toward agencies/enterprises rather than indie/small-account usage. **Don't run an actual generation through it until real pricing is confirmed and the founder has explicitly OK'd that specific spend** — same posture as `eromify`/`zencreator`'s founder-gating before those were removed, except the concern here is unknown cost, not content policy. Recommendation on file (2026-08-25): try `openart` for a given visual need first, since it's already trusted, already paid for, and already proven on this exact site (the DRYDOWN hero/chapter-4 videos) — reach for `motion` only if `openart`'s output genuinely doesn't do the job.
  - **Not yet connected**: like `fanvue`, this needs a live OAuth approval step (`/mcp` in an interactive Claude Code session) that a non-interactive session can't complete — `claude mcp list` / an actual `mcp__motion__*` tool call is how to check whether that's happened yet, don't assume it's live just because it's in `.mcp.json`.
- **Standing rule, not just a one-off correction**: never hand-draw/hand-code an SVG illustration as a page's animated visual centerpiece (product silhouettes, mascot-style graphics, decorative motifs) — always reach for AI-generated imagery/video (`openart`, or `motion` once its cost is confirmed) instead, even when the SVG route seems like the safer way to stay clear of the trademark-caution rule above. Solve that specific tension by generating an *abstract/artistic* AI image or video, not by switching to hand-drawn vector art.
