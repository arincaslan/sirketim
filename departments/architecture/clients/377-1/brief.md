...
**Date**: 2026-08-20
**Status**: Internal example/practice pass exercising the `project-brief` skill and `lib/cadgen` end-to-end against real intake — not a confirmed client engagement (see `CLAUDE.md`). Schematic depth tier.
...
### Bodrum Kat (Basement) — service/amenity, not full habitable bedrooms
...
### Zemin Kat (Ground Floor) — **the level actually generated as DXF/SVG/DWG this pass**
...
### 1., 2., 3. Normal Kat (Typical Upper Floors — bedroom/private zone)
...
**Honest limitation, not papered over**: `lib/cadgen/plan.py` is single-level only (its own module docstring already flags this as a known foundation-phase gap). This pass generated concrete DXF/SVG/DWG geometry for the **ground floor only** — the basement and 3 upper floors above are described in the room program above as the department's actual design proposal, but weren't drawn level-by-level, because the generator doesn't support that yet. Separately, the generator's only layout algorithm (single-row bay-slicing) forces every room on a level to share one depth — at this lot's resulting ~4.2m bay depth, a small standalone room (e.g., a bare guest WC at ~3 m²) would come out as an unrealistic sliver, so the concrete ground-floor realization consolidates into 4 larger zones rather than every individual room in the fuller conceptual breakdown. Both limitations are genuine, current gaps in the library — not something silently worked around — and are exactly the kind of finding the founder asked this pass to surface.

## 5. Open questions

1. **Client-registration status** — is 377/1 the founder's own property, a speculative project, or an actual client engagement? Nothing provided indicates which. Held out of `shared/clients.md` pending an answer (see `CLAUDE.md`).
