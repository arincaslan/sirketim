...
**SUPERSEDED AGAIN, not rewritten (2026-08-21, later pass)**: the program has moved again since the paragraph above — 6-unit apartment building -> **7-unit** building (maximized footprint toward the TAKS cap, one reverse-duplex unit at zemin+bodrum with its own private internal stair, two roof-duplex units at 3rd normal kat + çatı piyesi each with their own private internal stair). See `notes/rationale.md`'s dated 2026-08-21 section (§16) for the current program and full design reasoning (why 7 units, the parking-width conflict that caps the reverse duplex at exactly 1 unit, why the roof-duplex move is unit-count-neutral), and `CLAUDE.md`'s "Room program" section for the current room-by-room breakdown. **`notes/rationale.md` remains the current source of truth for this project's program — this brief is not**, for either the 6-unit or the 7-unit pivot. A full rewrite of this brief remains real, not-yet-done follow-up work, unchanged from the note above.
