# Amazon Associates — application preparation for DRYDOWN
...
Written 2026-08-26 · Site: `products/affiliate-sites/fragrance-dupes/` (DRYDOWN)
...
**Do not make Amazon Associates the first affiliate program DRYDOWN applies to. Apply to the
direct dupe-house programs first, and treat Amazon as a later, narrow add-on for the
*original* fragrances only.**

Three reasons, in order of weight:

1. **The products DRYDOWN actually recommends are not sold on Amazon.** Dossier, ALT.
...
- **Displayed prices must be refreshed at least every 24 hours and carry a "last updated"
  timestamp**, sourced via the Product Advertising API. DRYDOWN currently renders static
...
- **Redirecting/cloaking links.** The agreement bars obscuring the source site "including
  ...
  from. DRYDOWN routes every outbound click through `/go/[slug]`. This is *probably* fine
...
magnitude, and (c) Amazon simply does not carry the products DRYDOWN's whole comparison
...
already establishes that originals need their own link and that DRYDOWN should earn
...
| The form asks | How to answer for DRYDOWN |
|---|---|
...
| Website / app list | The live DRYDOWN domain. Do not list localhost, a preview URL, or anything not deployed. |
