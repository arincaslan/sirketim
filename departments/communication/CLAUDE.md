...
- **Amazon Associates is dropped, not deferred** (founder decision, 2026-08-26). Two independent blockers: Turkey is **invite-only** for the programme, *and* Amazon cannot pay a Turkish bank. Payoneer fixes the payout half and does nothing about the invite gate.

  **Correction, verified live 2026-08-27** against Amazon's own Associates help page: eligibility is by **country, not by account currency**. The list is United States (USD), United Kingdom (GBP/EUR) and 51 Eurozone/EEA countries (EUR) — 52 in total, and **Turkey is not among them**. This wording matters, and the earlier "by account currency (USD/GBP/EUR)" phrasing here caused a real misreading: the founder holds a **USD account** and reasonably concluded that cleared the blocker. It does not — a USD-denominated account *held in Turkey* is still ineligible, because the country is what is checked. Payoneer works precisely because it supplies a **US-domiciled** account, which is a different thing from a USD-denominated one. Re-check this at the source before reopening Amazon; do not restate it from memory. Separately CJ/FragranceX beats it on merits (10% vs 3%, 45-day vs 24-hour cookie), and Amazon is the only programme whose terms conflict with a `/go/[slug]` redirect.
