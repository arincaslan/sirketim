...
| **Awin** (COUNTERSCENT — planned first programme) | **Firm (A.Ş.)** — founder decision, 2026-08-27 | Bank transfer. **Awin does not self-bill Turkish tax residents** — Turkey is an explicit exception in Awin's own policy, so the publisher issues their own invoice per payout. | Not enrolled. Firm USD receiving account **not confirmed** — see the mismatch note. | The invoicing requirement is what settled the payee question: a Turkish individual cannot casually issue a *fatura*, so the payee is the A.Ş. **Open mismatch, needs resolving before enrollment:** the founder confirmed on 2026-08-27 that a USD account exists but is *not connected to Payoneer*, and the only USD account previously recorded here (2026-08-24) is a **personal** one. An A.Ş. issuing the invoice while a personal account receives the money means the invoice issuer and the payment recipient are different legal persons — a Turkish *mali müşavir* will object to that, and it is the precise form the CFO's "costs sit in the A.Ş., revenue lands personally" warning takes here. **Confirm whose name the USD account is in before enrolling**; if personal, the A.Ş. needs its own USD (döviz) account, which Turkish banks open routinely for a company with a tax ID on file. |
