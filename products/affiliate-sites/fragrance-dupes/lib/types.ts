...
export interface ReferenceFragrance {
   ...
   *  §1): DRYDOWN earns on originals too, not just dupes, so every reference
...
/**
 * A seller listing dupes on DRYDOWN. Producers are first-class as of the
 ...
 *
 * `isHouse` marks DRYDOWN's own fragrance line: no subscription, no approval
