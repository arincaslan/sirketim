...
**Current state, 2026-09-04, across THREE merchants:** `DUPES` holds **55 listings** across **41 references**, `affiliateLinks` holds **53 real Awin entries**, `REVIEWS` is still `[]`. All three were emptied on 2026-08-27 and the reasoning below is the bar every refill had to clear, not a superseded note.

| | Then | Now |
|---|---|---|
| `DUPES` | `[]` | **55 listings**, every name read off a live merchant feed *and* re-checked against the merchant's own page, each carrying `offers` with the exact rows |
| `affiliateLinks` | `{}` | **53 entries** — **30 Opulensi** (Awin 123248) + **9 Clone of Perfume** (117395) + **14 AromaPassions** (34989) — each traced AND stock-checked before being added |
| `REVIEWS` | `[]` | still `[]` — nothing has changed here, and inventing reviews is the worst of the three failures |

So `/go/<id>` resolves for exactly fifty-three ids and 404s for everything else. **Forty-six listings render a buy button**, not fifty-five: seven links track fine but their products are out of stock at the merchant (all seven Opulensi — Armaf Club de Nuit Intense Man; Fragrance World Neroli Riviera, Ard Al Zaafaran Oud Orchid, Lattafa Qaa'ed, Fragrance World Vanille En Tobacco, Lattafa Ameer Al Oudh Intense Oud, Zimaya Oud Is Great), so those rows show the price and "Out of stock" instead, and two listings are carried only by the closed My Perfume Shop programme. Enrolled, tracking, and in stock are three separate questions and all three have failed here. **All nine Clone of Perfume and all twenty-eight AromaPassions offers are genuinely in stock**, which is why the buyable count keeps rising faster than the listing count.

**Offers are per PRESENTATION, not per retailer, and those stopped being the same thing on 2026-09-04.** AromaPassions sells each product in 50ml and 100ml, so its listings carry two offers from one shop — 55 listings, 70 offers. `buy-actions.tsx` counts distinct merchants for its header rather than counting offers; it previously said "2 retailers - prices are each retailer's own" of one retailer's two bottles.
...
- **This is a scoring-formula question, not a data question, and it is unresolved.** Weighting declared-note overlap down, or discounting a pyramid identical to the reference's, would change every existing score — so it belongs in its own considered change alongside the `familyBonus` bug, not in a listings batch. Flag it to the founder rather than fixing it in passing.
...
So the ceiling is not "200 with more merchants". It is closer to **50–70 references**, and reaching even that needs several more dupe-side merchants. Judge progress by *originals covered* (**41**) rather than by listing count (**55**) — six listings on Aventus is depth, not reach, and the reach number is the one that matters for search traffic. **The AromaPassions batch of 2026-09-04 took coverage from 27 to 41 in one pass**, every one of its fourteen landing on an original with no alternative at all, which puts the estimated ceiling in sight and suggests it was, if anything, pessimistic.
...
The site now has **200 references and 55 listings across 41 references** (Aventus carries six competing listings, Oud Wood four, Angels' Share and Baccarat Rouge 540 three each, Oud for Greatness and Lost Cherry two each, and 35 originals have exactly one — which is what the ranking tie-breakers exist for), so **159** references still render the "alternatives" empty state — that is the normal case, not a bug. The house-product plumbing (`components/dupe-finder/house-badge.tsx`, the scoring constraint in `lib/verification.ts`) is intact and still has nothing to render: there are deliberately no house products.
...
**What changed, 2026-09-01, grew on 09-02, grew three times on 09-03 and again on 09-04:** `affiliateLinks` now holds **53** real, *tracking* dupe links across **three** merchants (30 Opulensi/123248 + 9 Clone of Perfume/117395 + 14 AromaPassions/34989), so a comparison or review anchored on any of 55 listings across 41 originals can name a real product with a real link. The two dupe houses that sell direct help disproportionately: every one of their offers is in stock, and between them they cover nineteen originals that had no alternative to write about at all. **Two things still stand between that and a published piece, and both fail loudly rather than quietly:**
...
| `DUPES` (55) | **53** | Opulensi (30), Clone of Perfume (9) **and** AromaPassions (14) → `scripts/fetch-dupe-images.mjs` → `lib/data/dupe-images.generated.ts` |
...
**Known open question, flagged rather than fixed:** the 156 reference images all came from the My Perfume Shop feed, which was live and approved when they were pulled and has since gone closed for tracking. By the rule above they are on weaker footing than the 53 dupe images, which came from Opulensi, Clone of Perfume and AromaPassions, all open and tracking. They are not removed because the enrolment was genuine at the time and we may yet get an answer from Awin about why 106089 closed — but if that programme stays dead, re-source them from a tracking merchant rather than leaving it.
...
**Images are committed — this section previously said they were not, and that was wrong.** `public/images/` is **209 files**; **195 are tracked** and the **14 AromaPassions files added 2026-09-04 are untracked pending review**. Re-measure rather than trusting either number:
