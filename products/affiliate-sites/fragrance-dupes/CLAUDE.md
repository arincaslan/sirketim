...
| Compare tier-agnostically when no pyramid exists | Not implemented. It is the honest representation — don't claim tier structure you were never given — but it changes every existing score, so it belongs with the `familyBonus` bug in one considered change. |
