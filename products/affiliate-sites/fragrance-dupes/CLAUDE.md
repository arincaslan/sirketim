...
**There is no escape hatch. The 95 ceiling is absolute, and that is a 2026-09-18 change.** Until then `DupeCandidate.founderOverride` (`{ score, note, date }`) could publish its own number past both ceilings, fenced by `validateFounderOverride()`, its own "Founder's personal assessment" badge, and a bar on house products. The founder removed it unused — zero listings ever carried one, confirmed by query before the schema columns were dropped — so no published score changed. `getPublishedScore` now has a single exit through `Math.min`, which is the whole check: **if you are looking for the condition under which a score exceeds 95, there isn't one.** Do not re-add it as a field; a personally-worn opinion belongs in the verdict prose with a byline, not in a number a reader cannot distinguish from a computed one. Full reasoning: `PRODUCER-PROGRAM.md` §7 item 5.
...
- **The retailer band on the home page is a DISCLOSURE, not a logo wall, and the wording is the whole point.** `components/home/retailer-band.tsx` names the five retailers we earn from, under a heading that says exactly that, with each retailer's own mark below its name. It must never become "Partners", "Sponsors", "As featured in" or anything similar: no such relationship exists in either direction, none of these companies has reviewed anything here, and several of them sell products this site rates against each other. Framed honestly it strengthens the independence claim; reframed as a partner strip it would assert an association we do not have. `shared/clients.md` records that Sirketim currently has **no third-party clients at all** - every entry is internal - so there is nobody whose logo could legitimately appear under a partner heading. **Three rules keep the marks defensible and they are load-bearing, not styling:** the NAME always stays above the mark (a mark alone discloses nothing - AromaPassions' is a bare lowercase "a"), no mark is ever restyled beyond being painted one colour through a CSS mask, and the marks are self-hosted rather than hot-linked, because the networks' creative URLs are impression trackers that would put a third-party request on the home page of a deliberately cookieless site. Provenance for each of the five is recorded in `lib/merchants.ts` - and note that a network "creative" is often a promo banner rather than a logo (Opulensi's is), so check before preferring it. **Never AI-generate or retouch one**: an altered logo misrepresents a real company and an invented one is worse.
- **The producer revenue model is DECIDED (2026-09-10) and AMENDED (2026-09-18): free tier of ONE listing, and NO COMMISSION ON ANY TIER, THE FREE ONE INCLUDED.** Every producer's sales earn us nothing at any tier; a subscription is the only thing a producer ever pays us. `lib/plans.ts` carries `takesCommission` per tier (now `false` three times) and `tiersWithoutCommission()` (now every plan), because the publish path has to know whether a listing gets an affiliate link or a direct store link - and the answer is now always "direct". A direct link has no `/go/` sub-ID, so **all** producer click data must come from our own logging, not a network's reporting. **Do not reintroduce commission on any tier to lift revenue**, and note the free tier is where that temptation now lives, since it earns nothing at all and is justified purely as a funnel to paid. The whole of `PRODUCER-PROGRAM.md` section 7 rests on "no tier buys rank", and the cleanest version of that argument is that we earn nothing from where any producer ranks. Prices (19/49) are still placeholders; nobody has done section 3's research. **This is separate from the catalogue's own affiliate revenue from retailers**, which is untouched and is what funds the site.
- **A checker that silently covers a subset is worse than no checker, because it converts "we have not looked" into "we looked and it was fine".** `check-affiliate-links.mjs` kept its own copy of the link-source list and fell two files behind `generate-redirects.mjs`: 368 of 620 shipped links - every Perfumania link, both namespaces - went to the edge unchecked while the script reported a clean pass, under a comment asserting the two lists matched. The list now lives once, in `scripts/lib/affiliate-link-sources.mjs`, and both import it. **Whenever one script verifies what another produces, they must read the same source list from the same place - a comment claiming they agree is not a mechanism.** Two smaller bugs fell out of fixing it and are the same species: Perfumania was never added to `MERCHANT_NOTES`, and the summary line printed "Awin click cookie" for both CJ merchants whose own notes directly below said the Awin channel does not apply.
- **Every link check is a real affiliate click, so coverage and request volume are separate decisions.** `check:links` now samples 12 per merchant by default and prints what it skipped; `--all` opts into all 620. Firing 620 sequential clicks from one IP is a pattern a network can reasonably read as click fraud, and the penalty is account termination rather than a warning - so widening what a script *knows about* must not silently widen what it *requests*.
- **Never let the party being measured supply BOTH inputs to the check that measures them.** `components/producers/submission-form.tsx` collects six facet scores from the producer, and `isVerbatimCopy()` flags a copy only when notes AND facets both match. That gate works today solely because facets are ours. Hand the same party both and it is defeated by construction - copy the reference's notes, nudge one facet past `FACET_EPSILON`, and nothing fires, which lets any producer reach the 90 cap and rank first on their own reference. **Rank would then be purchasable in substance while remaining unpurchasable in letter**, with `/producers` promising the public the opposite. Removing those sliders is build step 1 in HANDOFF; facets get derived by us, as they already are for the three listed merchants.
...
**A PAGE THAT SETS NO `alternates` INHERITS THE LAYOUT'S CANONICAL, WHICH POINTS AT THE HOMEPAGE - so it silently tells Google it is a duplicate of `/`.** `app/layout.tsx` carries `alternates: { canonical: "/" }` so that nothing ships canonical-less, and that default is right for the homepage and wrong for every other page that forgets to override it. Nine did: `/about`, `/disclosure`, `/library`, `/privacy`, `/producers`, `/producers/pricing`, `/producers/login`, `/producers/submit` and the 404. Search Console reported them under "alternate page with proper canonical tag" - which reads like an informational notice and is actually the site asking, in its own markup, not to be indexed. Fixed 2026-09-16. **Audit this against the BUILT OUTPUT, not the source**, because the inheritance is invisible in a page file - walk `out/**/*.html` and assert each page's `rel=canonical` equals its own URL. 242 of 244 pass; the two failures are `/404/`, which is correct.

**`trailingSlash: true` means every internal link must carry the slash, and 33 did not** - each served a 307 to the slashed form, which Search Console reports as "page with redirect" and does not index. The built output is the check here too: `grep -rhoE 'href="/[a-z0-9][a-z0-9/-]*"' out --include='*.html' | grep -v '/go/' | grep -vE '/"$'` must return nothing. **`/go/` links are excluded on purpose** - they are `_redirects` rules, not pages, and a trailing slash breaks all 620. Note the obvious version of that grep silently passes: `/` is inside the character class, so the pattern matches correctly-slashed hrefs too and reports a clean 240 when it has checked nothing.
