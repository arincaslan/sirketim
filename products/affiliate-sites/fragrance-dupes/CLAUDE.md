...
- **The producer revenue model is DECIDED (2026-09-10) and the unusual half is the load-bearing half: free tier of ONE listing, and NO COMMISSION ON ANY PAID TIER.** A subscriber pays a fee and their sales earn us nothing; the free tier is commission-only. `lib/plans.ts` carries `takesCommission` per tier and `tiersWithoutCommission()`, because the publish path has to know whether a listing gets an affiliate link or a direct store link - and a direct link has no `/go/` sub-ID, so subscriber click data must come from our own logging, not a network's reporting. **Do not reintroduce commission on a paid tier to lift revenue.** The whole of `PRODUCER-PROGRAM.md` section 7 rests on "no tier buys rank", and the cleanest version of that argument is that we earn nothing from where a subscriber ranks. Prices (19/49) are still placeholders; nobody has done section 3's research.
