...
**Current state, 2026-09-03, across TWO merchants:** `DUPES` holds **41 listings** across **27 references**, `affiliateLinks` holds **39 real Awin entries**, `REVIEWS` is still `[]`. All three were emptied on 2026-08-27 and the reasoning below is the bar every refill had to clear, not a superseded note.

| | Then | Now |
|---|---|---|
| `DUPES` | `[]` | **41 listings**, every name read off a live merchant feed, each carrying `offers` with the exact feed rows |
| `affiliateLinks` | `{}` | **39 entries** — **30 Opulensi** (Awin 123248) + **9 Clone of Perfume** (Awin 117395) — each traced AND stock-checked before being added |
| `REVIEWS` | `[]` | still `[]` — nothing has changed here, and inventing reviews is the worst of the three failures |

So `/go/<id>` resolves for exactly thirty-nine ids and 404s for everything else. **Thirty-two listings render a buy button**, not thirty-nine: seven links track fine but their products are out of stock at the merchant (all seven Opulensi — Armaf Club de Nuit Intense Man; Fragrance World Neroli Riviera, Ard Al Zaafaran Oud Orchid, Lattafa Qaa'ed, Fragrance World Vanille En Tobacco, Lattafa Ameer Al Oudh Intense Oud, Zimaya Oud Is Great), so those rows show the price and "Out of stock" instead. Enrolled, tracking, and in stock are three separate questions and all three have failed here. **All nine Clone of Perfume offers are genuinely in stock**, which is why the buyable count jumped further than the listing count.
...
So the ceiling is not "200 with more merchants". It is closer to **50–70 references**, and reaching even that needs several more dupe-side merchants. Judge progress by *originals covered* (**27**) rather than by listing count (**41**) — six listings on Aventus is depth, not reach, and the reach number is the one that matters for search traffic.
...
The site now has **200 references and 41 listings across 27 references** (Aventus carries six competing listings, Oud Wood four, Angels' Share and Baccarat Rouge 540 three each, Oud for Greatness and Lost Cherry two each, and 21 originals have exactly one — which is what the ranking tie-breakers exist for), so **173** references still render the "alternatives" empty state — that is the normal case, not a bug. The house-product plumbing (`components/dupe-finder/house-badge.tsx`, the scoring constraint in `lib/verification.ts`) is intact and still has nothing to render: there are deliberately no house products.
...
**What changed, 2026-09-01, grew on 09-02, and grew three times on 09-03:** `affiliateLinks` now holds **39** real, *tracking* dupe links across **two** merchants (30 Opulensi/123248 + 9 Clone of Perfume/117395), so a comparison or review anchored on any of 41 listings across 27 originals can name a real product with a real link. The second merchant helps here disproportionately: all nine of its offers are in stock, and it covers five originals that had no alternative to write about at all. **Two things still stand between that and a published piece, and both fail loudly rather than quietly:**
...
| `DUPES` (41) | **39** | Opulensi feed (30) **and** Clone of Perfume feed (9) → `scripts/fetch-dupe-images.mjs` → `lib/data/dupe-images.generated.ts` |
...
**Known open question, flagged rather than fixed:** the 156 reference images all came from the My Perfume Shop feed, which was live and approved when they were pulled and has since gone closed for tracking. By the rule above they are on weaker footing than the 39 dupe images, which came from Opulensi and Clone of Perfume, both open and tracking. They are not removed because the enrolment was genuine at the time and we may yet get an answer from Awin about why 106089 closed — but if that programme stays dead, re-source them from a tracking merchant rather than leaving it.
...
**Images are committed — this section previously said they were not, and that was wrong.** `public/images/` is **195 files, ~23MB**; **186 are tracked** (committed in `e7f524a`, along with both `lib/data/*-images.generated.ts` manifests) and the **9 Clone of Perfume files added 2026-09-03 are untracked pending review**. Re-measure rather than trusting either number:
