...
**Current state, 2026-09-03 (final):** `DUPES` holds **32 listings** across **22 references**, `affiliateLinks` holds **30 real Awin entries**, `REVIEWS` is still `[]`. All three were emptied on 2026-08-27 and the reasoning below is the bar every refill had to clear, not a superseded note — **read it before proposing a thirty-third from this merchant; there isn't one, see below.**

| | Then | Now |
|---|---|---|
| `DUPES` | `[]` | **32 listings**, every name read off a live merchant feed, each carrying `offers` with the exact feed rows |
| `affiliateLinks` | `{}` | **30 entries**, all Opulensi (Awin 123248), each traced AND stock-checked before being added |
| `REVIEWS` | `[]` | still `[]` — nothing has changed here, and inventing reviews is the worst of the three failures |

So `/go/<id>` resolves for exactly thirty ids and 404s for everything else. **Twenty-three listings render a buy button**, not thirty: seven links track fine but their products are out of stock at the merchant (Armaf Club de Nuit Intense Man; Fragrance World Neroli Riviera, Ard Al Zaafaran Oud Orchid and Lattafa Qaa'ed, added 2026-09-03; and — the final batch, also 2026-09-03 — Fragrance World Vanille En Tobacco, Lattafa Ameer Al Oudh Intense Oud and Zimaya Oud Is Great), so those rows show the price and "Out of stock" instead. Enrolled, tracking, and in stock are three separate questions and all three have failed here, on seven of thirty linked offers so far.
...
So the ceiling is not "200 with more merchants". It is closer to **50–70 references**, and reaching even that needs several more dupe-side merchants. Judge progress by *originals covered* (22) rather than by listing count (32) — five listings on Aventus is depth, not reach, and the reach number is the one that matters for search traffic.

**How far this feed goes, measured 2026-09-02, re-checked and closed out 2026-09-03 — do not re-derive the underlying scan.** 610 rows → 533 matched to the storefront → **245 actually in stock** (the feed marks all 610 `in_stock=1`) → 218 with a parseable note pyramid → **only 21 name a fragrance that is in REFERENCES**, and several of those are false positives. Chasing "impression" (225 rows) and "TF" (67) yielded nothing: the first is ordinary English, the second a substring bug. The in-stock pool is dominated by Anfar, Adyan, Sapil and Al-Rehab, whose ranges are original Arabian compositions rather than designer clones. **This merchant tops out at 32 listings covering 22 originals** — two more passes over the same 610 rows (2026-09-02, then again 2026-09-03) found seven more genuine pairings total (Neroli Riviera, Oud Orchid, Qaa'ed, Mohra, then Vanille En Tobacco, Ameer Al Oudh Intense Oud, Zimaya Oud Is Great) and turned up nothing further to chase; that is the realistic end of this feed, not a snapshot to re-scan. **Growing past 32/22 needs a second dupe-side merchant — that is a separate future task, not more work on Opulensi.**
...
The site now has **200 references and 32 listings across 22 references** (Aventus carries five competing listings, Angels' Share and Oud Wood three each, Oud for Greatness and Baccarat Rouge 540 two each — which is what the ranking tie-breakers exist for), so 178 references still render the "alternatives" empty state — that is the normal case, not a bug. The house-product plumbing (`components/dupe-finder/house-badge.tsx`, the scoring constraint in `lib/verification.ts`) is intact and still has nothing to render: there are deliberately no house products.
...
**What changed, 2026-09-01, grew on 09-02, and grew again (twice) on 09-03:** `affiliateLinks` now holds **30** real, *tracking* dupe links (all Opulensi, Awin 123248), so a comparison or review anchored on any of 32 listings across 22 originals can name a real product with a real link. **Two things still stand between that and a published piece, and both fail loudly rather than quietly:**
...
| `DUPES` (32) | **30** | Opulensi feed → `scripts/fetch-dupe-images.mjs` → `lib/data/dupe-images.generated.ts` |
...
**Known open question, flagged rather than fixed:** the 156 reference images all came from the My Perfume Shop feed, which was live and approved when they were pulled and has since gone closed for tracking. By the rule above they are on weaker footing than the 30 dupe images, all of which came from Opulensi, a programme that is still open and tracking. They are not removed because the enrolment was genuine at the time and we may yet get an answer from Awin about why 106089 closed — but if that programme stays dead, re-source them from a tracking merchant rather than leaving it.
...
- **`affiliateLinks` holds 30 real entries as of 2026-09-03 (the final batch from this feed)**, all keyed `dupe-<brand>-<product>`, so exactly those 30 `/go/<slug>` paths resolve and every other `/go/<anything>` 404s. Both halves are correct. Confirm the count from the file, not from here — this line has been stale more than once already, and `grep -c 'deepLink:' lib/affiliate-links.ts` is itself an unreliable way to check it: that pattern also matches the `deepLink: string` field on the `AffiliateLinkEntry` interface and the one hand-built inside `resolveAffiliateLink()`'s placeholder fallback, so it over-counts by two (returns 32, not 30). Read the real number off `node scripts/check-affiliate-links.mjs`'s own header line (`checking N link(s)`) instead — it parses this file with the same regex `generate-redirects.mjs` uses to build `_redirects`. Buy buttons refuse to render unless `hasRealAffiliateLink()` resolves, so no visitor can reach a placeholder. The generator **fails the build** if an entry lacks a `deepLink` or `subId`, or names a network with no sub-ID parameter — an untagged click is unattributable forever, so it must never ship quietly.
