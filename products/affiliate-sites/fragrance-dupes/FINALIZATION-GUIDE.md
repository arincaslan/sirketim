...
| **Producer console** | **LIVE 16 Sep** — three states, a listings table, a quota line, `/console/submit` (the listing form, with `<datalist>` note selectors drawn from our own vocabulary) and `/console/withdraw`. Both answer 401 unauthenticated, verified from outside. Withdraw sets `WITHDRAWN_BY_PRODUCER` and never deletes the row | ✅ |
| **Review queue** | **DOES NOT EXIST — and it is the next build.** A submission lands `PENDING` and *nothing can move it out of that state*. `/review` is `notShipped()`, makes no database calls, and returns **200 to anyone** — safe only while it stays inert. Needs a staff role (`User` has no `role`/`isStaff`; `ActorType.STAFF` exists in the enum but nothing can be identified as staff), access control in the same change as its first query, and the queue itself | ❌ **next** |
...
| **5.7** | **Build the review queue — the next build, and the loop is half open until it lands.** Three things in ONE change, not sequentially: (a) a **staff role**, which is a migration — `User` has no `role`/`isStaff` column, and `ActorType.STAFF` at `schema.prisma:251` means the audit trail anticipates staff acting without anything being able to identify one; (b) **access control on `/review`**, mandatory in the same change as its first database call and never after, because the moment that route reads anything real a public 200 stops being a harmless placeholder; (c) the **queue** — `PENDING` oldest first, approve/reject with a written reason, every action an `AuditEvent`. Migration goes **before** the deploy, the same ordering that made `/sign-in` fail closed with a 503 when its table was missing | **Now** |
