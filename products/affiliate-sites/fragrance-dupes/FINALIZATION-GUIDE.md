...
| Sitemap entries | **221+** (was ~11), all trailing-slash canonical — re-measure with `curl -s https://counterscent.com/sitemap.xml \| grep -c "<loc>"` rather than trusting this row | ✅ |
...
| Working affiliate links | **252 `/go/` ids** — 77 dupe-side (Opulensi 30, Clone of Perfume 9, AromaPassions 38), plus **116 `original-*` and 59 `shop-*`** from FragranceShop.com (CJ). Dupe links traced *and* stock-checked; the CJ destinations are **inconclusive by WAF**, not passing — see 3.3 | ✅ |
...
| CJ Affiliate | **FragranceShop.com (16941446) delivered 7 Sep and fully wired** — 5,802 rows ingested, 103 buy links, 116 reference photographs, `/originals` live. **Perfumania is still approved-but-unwired: we are waiting on them to send the feed** (as of 8 Sep). | ◐ Perfumania |
| Merchants we can actually earn from | **4 wired** — Opulensi, Clone of Perfume, AromaPassions (Awin, dupe side) + FragranceShop.com (CJ, originals side). **1 approved, waiting on their feed:** Perfumania | ✅ |
| Revenue to date | **$0** | — |
