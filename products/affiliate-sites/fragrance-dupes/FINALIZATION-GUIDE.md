...
| Producer dashboard | Route exists, hardcodes "signed out" for everyone. **A producer who signs in lands in an empty room** | ❌ next |
| Payment rail for producers | **None.** Stripe/PayPal/Gumroad do not serve Turkey; Paddle/iyzico/PayTR/Payoneer are the options. Plan prices ($19/$49) are a considered guess, not a price | ❌ |
| Scoring integrity | House products capped; rank cannot invert (27 Aug). **Extended 8 Sep: a 95% structural ceiling on every score including `verified`, a −10 penalty where the note-tier split is ours not the seller's (47 of 79 listings), `familyBonus` fixed, and a 15% ingredient component that stays inert until both sides have a list.** A disclosed founder override is the only way past 95 | ✅ |
...
| **5.5** | **Producer dashboard — the next build.** `/console` exists and hardcodes "signed out". Add-listing, note selectors, delete, and the owner's approval view all sit behind it | **Now — auth is live and there is nothing behind it** |
| **5.6** | **DECIDE: human approval queue vs automated rule-checking.** Founder deferred this deliberately on 16 Sep. `ActorType.AUTOMATION` already exists in the schema, which anticipates but does not settle it | Before 5.5 ships a submit path |
...
>
> **A disclosed founder override is the only way past 95** — a human number with a mandatory written justification, its own badge, never on a house product, and unable to rescue a flagged copy. Zero listings use it.
>
