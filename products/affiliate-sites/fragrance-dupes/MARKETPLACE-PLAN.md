...
- **Subscription billing** — recurring payment collection from producers. Nothing payment-related is configured anywhere in this repo today. **This line used to name Stripe as "the obvious default"; it is not an option at all** — Stripe does not serve a Turkey-based business, and the Stripe integration that once lived here was deleted 2026-08-27 for exactly that reason. The board's recommendation is **Paddle** (merchant of record), and explicitly **not iyzico**, which is capable of recurring billing but leaves every tax and invoicing obligation on us while billing US producers in TRY. See `departments/accounting/reports/payment-rails-investigation.md`.
...
- **Payment processor choice** — **Paddle**, recommended by the board 2026-09-10 and still to be confirmed against Paddle's marketplace acceptable-use policy before any adapter is written. Not Stripe (does not serve Turkey), not iyzico (wrong shape for a US-billed subscription).
