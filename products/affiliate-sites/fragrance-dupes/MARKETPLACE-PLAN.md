# DRYDOWN marketplace pivot — plan
...
- **DRYDOWN / Sirketim** reviews every submission before it's visible on the site — a moderation gate, not auto-publish. The founder does the approve/reject call personally, at least at this stage.
...
2. **Commission on sales** — DRYDOWN earns a cut (affiliate commission, or a direct commission if DRYDOWN brokers the transaction itself) on every purchase made through the site, regardless of which option the buyer picks: a producer's dupe, the original fragrance, or DRYDOWN's own line. This means the **original** fragrance needs a real buy/affiliate link on every comparison, not just the dupes — today's `ReferenceFragrance` type (`lib/types.ts`) has no link field at all, only `DupeCandidate` does (`affiliateLinkId`).
3. **First-party margin** — DRYDOWN's own fragrance line sells directly (or via whatever checkout/fulfillment path gets chosen, §5) with a known cost basis: $8.51 per 55ml bottle. This is the one catalog entry that isn't affiliate-driven — it's an actual product with real margin.

**What's still open**: exact subscription pricing, commission percentage, and whether DRYDOWN ever takes on real transaction/payment liability (vs. staying pure affiliate-link cloaking) — see §5.
...
- **DRYDOWN's own product** — a distinct entity type from a producer Listing: no subscription, no approval workflow (it's the house brand), but its own cost/margin fields (starting from the $8.51/55ml figure) rather than an `affiliateLinkId`.
...
- **Affiliate-link cloaking at scale** — the existing `/go/[slug]` redirect pattern (`app/go/[slug]/route.ts`, shared via `departments/web-development/lib/affiliate-site-kit/`) already does link cloaking for one dataset; it needs to extend cleanly to many producers' many links, plus however DRYDOWN tracks its own commission/attribution on top of that (click tracking, conversion tracking — neither exists today).
...
- **Commission percentage** — on producer sales, and (if DRYDOWN ever brokers original-fragrance or its own-line sales directly) on those too.
...
- **How DRYDOWN's own line actually sells** — through the site's own checkout (a real e-commerce build, which this site explicitly doesn't have today — see README's "no backend, no database, no cart"), or by routing to an external store/marketplace listing instead?
