...
Items 2 and 4 are founder-side applications. **The company's registered *faaliyet konusu* was the open question here and it is now ANSWERED (2026-09-11):** the mali musavir's position is that the transactions get carried out and recorded, and if the activity code becomes an issue the tax office raises it and the code is added then. It is not a blocker. What it does is move the weight onto bookkeeping — `departments/accounting/CLAUDE.md` now carries a `Subscription revenue — <Product>` income category, with the warning that the `Type` column already uses `Subscription` for an expense we PAY, so filing revenue there subtracts it from the balance.
